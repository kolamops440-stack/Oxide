...
# ========== КЛАВИАТУРЫ С PREMIUM ЭМОДЗИ (Через HTML теги) ==========

def main_menu_keyboard():
    buttons = [
        [
            InlineKeyboardButton(text='<tg-emoji emoji-id="5208513917965328345">🛍️</tg-emoji> Каталог', callback_data="menu_catalog"),
            InlineKeyboardButton(text='<tg-emoji emoji-id="5886412370347036129">👤</tg-emoji> Профиль', callback_data="menu_profile")
        ],
        [
            InlineKeyboardButton(text='<tg-emoji emoji-id="5983399041197675256">📦</tg-emoji> Мои покупки', callback_data="menu_purchases")
        ]
    ]
...
def admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5208422125924275090">🔑</tg-emoji> Выдать ключ', callback_data="admin_give_key")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5805532930662996322">✅</tg-emoji> Подтвердить UAH', callback_data="admin_confirm_uah")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5877629862306385808">🔙</tg-emoji> В главное меню', callback_data="back_to_main")]
    ])

def catalog_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5312048193444282508">🎮</tg-emoji> Oxide Survival Island', callback_data="game_oxide")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5877629862306385808">🔙</tg-emoji> В главное меню', callback_data="back_to_main")]
    ])

def products_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5208422125924275090">🔥</tg-emoji> Lebro [VIP]', callback_data="product_Lebro_VIP")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5208422125924275090">⚡</tg-emoji> Lebro [Lite]', callback_data="product_Lebro_Lite")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5877629862306385808">🔙</tg-emoji> Назад к играм', callback_data="back_to_catalog")]
    ])

def periods_keyboard(product):
    buttons = []
    for name, code in PERIODS[product]:
        buttons.append([InlineKeyboardButton(text=f'<tg-emoji emoji-id="5985596818912712352">⏳</tg-emoji> {name}', callback_data=f"period_{code}")])
    buttons.append([InlineKeyboardButton(text='<tg-emoji emoji-id="5877629862306385808">🔙</tg-emoji> Назад к продуктам', callback_data="back_to_products")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def payment_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5361914370068613491">🪙</tg-emoji> CryptoBot (USDT)', callback_data="pay_crypto")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5805532930662996322">💳</tg-emoji> Оплата гривной', callback_data="pay_uah")],
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5877629862306385808">🔙</tg-emoji> Назад к периодам', callback_data="back_to_periods")]
    ])

def cancel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5985346521103604145">❌</tg-emoji> Отмена', callback_data="cancel")]
    ])

def agreement_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='<tg-emoji emoji-id="5985346521103604145">🤝</tg-emoji> Я ознакомлен с правилами', callback_data="agree")]
    ])
...
async def game_oxide(callback: types.CallbackQuery, state: FSMContext):
    ...
    text = '<tg-emoji emoji-id="5819078828017849357">⚙️</tg-emoji> <b>Oxide Survival Island - Выберите софт:</b>'
...
async def pay_crypto(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Реєструємо автоматичну покупку в базі
...
@dp.message(Command("admin"))  # Додано текстову команду для адміна, щоб зайти в панель
...
async def send_key(message: types.Message, state: FSMContext):
    ...
    # Перевірка: чи йде активація через підтвердження UAH
    ...
    try:
        ...
        await message.answer(f"⚠️ Ошибка отправки пользователю. Ключ сохранен в базу.\n\nКлюч: <code>{key}</code>", parse_mode="HTML")
