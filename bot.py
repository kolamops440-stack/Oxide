...
<tg-emoji emoji-id="5877465816030515018"> </tg-emoji> <b>Ссылка для оплаты (ДЕМО):</b> Оплата демо-режиме

<tg-emoji emoji-id="6005843436479975944"> </tg-emoji> <b>Ваш ключ:</b> <code>{auto_key}</code>"""
...
async def admin_confirm_uah(callback: types.CallbackQuery, state: FSMContext):
    ...
    for pid, info in data["pending_uah"].items():
        builder.button(text=f"✅ {info['username']} - {PRODUCT_NAMES[info['product']]} ({info['period']})", callback_data=f"confirm_{pid}")
    builder.button(text="🔙 Назад", callback_data="menu_admin")
    builder.adjust(1)
    
    await callback.message.edit_text("📋 <b>Выберите оплату для подтверждения:</b>", parse_mode="HTML", reply_markup=builder.as_markup())
    await callback.answer()
